...
### Fixed
- `retention`: 관형사 '한'·'하나'(= a/one)를 숫자 1로 오인식하던 false-positive 제거
  — `_KO_NUM`에서 제외('한 번/하나의'는 '단일/일괄'로 의미 보존되는데 수치 누락으로 오탐).
  실데이터(현대차 강의덱) 검증에서 9건 재현 → 제거. 명확한 수사(두/세/…)는 유지. 테스트 90→92.
