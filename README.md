# GAEJO — 한국 연구 발표 개조식 변환·검증 도구 v0.1.0

> 슬라이드 구어체·완전문장을 **개조식(箇條式 — 명사·명사형 종결의 압축 불릿 문체)**으로 다듬는다.
>
> **Claude Code 스킬 + `gaejo` CLI** 형태 — 에이전트(LLM)가 변환하고, GAEJO는 ① 개조식 규칙(스킬)과
> ② 결정론적 검증(`gaejo check`)을 제공한다. **API 키 불필요**(MCP는 선택).
...
- 한국 학술/연구 발표 슬라이드는 완전문장이 아니라 **명사·명사형 종결의 압축 불릿(개조식)**을 쓴다.
- LLM은 한국어 슬라이드를 종종 완전문장체로 쓰거나 영어식 문장형으로 옮긴다 → 발표 관례와 어긋남.
- GAEJO는 이 변환을 규칙화하고, **형태소 분석으로 결정론적으로 검증**한다(영어권 도구·BLEU/ROUGE로는 못 잡는 영역).
...
- 변환 품질은 **백엔드 에이전트(LLM)**에 달려 있다. GAEJO는 변환을 *검증·교정*할 뿐 생성하지 않는다.
- `check`의 의미보존 검사는 형태소 휴리스틱이다 — 수치·전문용어·뉘앙스(근사/역접/강조/가능성) 손실은
  잡지만, **헤드라인 의문형·인용 예시·연구질문을 완전문장으로 오탐**할 수 있다. 이런 항목은 예외로 둔다
  (스킬이 안내). 주어 누락 등 더 깊은 의미 손실은 객관 검사 범위 밖(LLM 판정 축의 몫).
...
- Claude (Anthropic) — 페어 프로그래밍·리서치 보조
